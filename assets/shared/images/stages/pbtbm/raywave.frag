...
vec3 makeClr(vec2 fragCoord){
    vec3 viewDir = rayDirection(60., resolution.xy, fragCoord);
...
void main() {
	vec4 cur = texture2D(sampler0, textureCoord);
		...
		for(int y = 0; y < AA; ++y)
			for(int x = 0; x < AA; ++x){
				fragColor.rgb += clamp(makeClr(imageCoord * resolution + vec2(x, y) / float(AA)), 0., 1.);
			}
		
		fragColor.rgb /= float(AA * AA);
		fragColor = fragColor * alpha + cur;
	} else {
		fragColor = cur;
	}
}